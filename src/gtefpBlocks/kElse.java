...
public class kElse extends Closure implements Serializable
{
	...
	public kElse(WorkspacePanel wp)
	{
		super(wp);
		_pElse = new kVar(wp, "else(");
		_pElse.setEditable(false);
		_pElse.setCp(this);
		_curly.setText("){");
		_eval = new Socket(wp);
		_eval.setCp(this);
		_eval.setRel(_pElse.width(),0);
		_after = new Socket(wp);
		_after.setCp(this);
		_after.setRel(0,_pElse.ySize()+_inside.ySize());
		_curly.setRel(_pElse.width() + _eval.width() , 0);
		_c= new java.awt.Color(110,210,230);
		_code.que(_pElse); _code.que(_eval); _code.que(_curly); _code.que(_ylruc); _code.que(_after);
	}
	@Override
	public void unplug(){
		super.unplug();
	...
	public void draw_p()
	{
		int[] w = new int[]{_curly.width() + _pElse.width() + _eval.width()+1, 8, _ylruc.width()};
		int[] h = new int[]{_curly.ySize(), _curly.ySize() + _inside.ySize(), _curly.ySize() + _inside.ySize() + _ylruc.ySize()};
		super.draw_p(w, h);
		_eval.setRel(_pElse.width(),0);
		_curly.setRel(_pElse.width()+_eval.width(),0);
		_pElse.setRel();
		_after.setRel(0,_pElse.ySize()+_inside.ySize()+_ylruc.ySize());
	}
